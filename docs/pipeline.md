...
- `segments/split` 现在不再对每个大片段自动再调一次 Gemini
- split 阶段的大片段卡内容直接来自整片分析返回的 `time_anchors`
- 片段级 Gemini 只保留给 `POST /api/segments/:id/analyze` 这个手动刷新入口
- 这样前端点击 `生成片段` 后，等待的是本地切片和资产构建，不再等待整段片段级 AI 二次分析
