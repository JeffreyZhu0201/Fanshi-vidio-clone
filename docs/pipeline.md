...
- 后端重启后恢复在途生成任务
- 对白镜头按真实对白时长动态放宽生成时长
- 只在超过 Seedance 单次上限时才有限度加速参考音频
- 压缩后字幕时间同步缩放
- 压缩倍率超过 `1.5x` 时阻止生成
- 对白镜头生成后返回 `requested/provider/actual duration`
- 片段压缩包导出
- 片段压缩包 `manifest.json`
- 成片拼接与下载
...
- 视频生成主要依赖角色三视图、场景参考图和展开后的资源提示词
- 原片视频默认不发送，只在用户手动打开“参考原片视频”时作为动作节奏、站位和机位参考加入
- 即使打开了原片视频，人物外观和场景外观也仍以资源三视图和资源提示词为主真值
- 典型帧默认发送，但只负责站位、景别、机位朝向、前后景层次和动作瞬间参考，不主导人物外观和场景美术

当前片段卡还新增了一个 `参考原片视频：开 / 关` 按钮：

- 默认：关
- 开：发送大片段源视频或小镜头源视频给 Seedance
- 关：不发送原片源视频，但仍保留三视图、场景图、典型帧、背景资产视频和参考音频

当前片段卡现在还新增了一个 `参考典型帧：开 / 关` 按钮：

- 默认：开
- 开：发送大片段典型帧或小镜头典型帧给 Seedance
- 关：不发送典型帧，但仍保留三视图、场景图、背景资产视频、参考音频和可选原片视频
...
- 先计算对白完整说完真正需要的时长
- `requestedGenerationDurationSeconds = max(原镜头时长, 对白所需时长 + 收口余量)`
- 只要没超过 Seedance 单次上限，就不再把对白强压回原镜头秒数
- 只有当对白总时长超过 Seedance 单次上限时，才会对参考音频做有限度加速
- 字幕时间同步缩放
- 超过 `1.5x` 需要压缩时，直接阻止生成
- 下载远端结果后，如果当前镜头要求生成对白但结果文件没有音轨，会直接判失败
- 下载远端结果后，如果真实视频时长明显短于对白完成目标，也会直接判失败
...
1. 同一大片段下全部小镜头成功
2. 按 `shotIndex` 顺序拼接真实生成文件
3. 不再假设每个小镜头长度等于原定义时长
4. 产出新的大片段结果
5. 大片段结果会带上真实 `actual_duration_seconds`
6. 把该结果作为大片段主结果返回给前端和 merge

### 3.11 片段压缩包导出

- 接口：
  - `POST /api/exports/segments/start`
  - `GET /api/exports/segments/:taskId/progress`
  - `GET /api/exports/segments/:taskId/download`

当前导出规则：

- 同时导出：
  - `segments/` 下的大片段主结果
  - `shots/segment-xx/` 下的小镜头真实成功结果
- 允许部分成功导出
- 只要至少有一个真实生成文件就可以打包
- 压缩包内固定附 `manifest.json`
- `manifest.json` 会列出：
  - 已打包项
  - 缺失项
  - 来源任务 ID
