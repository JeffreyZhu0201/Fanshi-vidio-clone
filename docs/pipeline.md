...
如果走首页的`一键出片`按钮，当前固定流程会更收敛：

1. 整片理解
2. 生成角色三视图和场景参考图
3. 切大片段和小镜头
4. 直接按当前大片段提示词和当前小镜头提示词批量生成
5. 自动拼接成片
...
- 已通过：`video_id=900000054` 的整片分析，返回 `remote-gemini / gemini-2.5-pro / bearer+query-key+node`
...
- 角色三视图要替换原片中的人物外观
- 场景参考图要替换原片中的空间和布景外观
- 小镜头最终提示词里已经写明的人物左中右位置、站姿/坐姿、朝向、视线和动作阶段现在按硬约束执行；没有明确写出换位、坐下、起身时，生成结果不能擅自改位或改姿态
...
3. 先把每个生成镜头统一转成可稳定拼接的音视频规格，并保留已有生成音轨
4. 不再假设每个小镜头长度等于原定义时长
5. 产出新的大片段结果
6. 大片段结果会带上真实 `actual_duration_seconds`
7. 把该结果作为大片段主结果返回给前端和 merge
...
- 优先用新生成的大片段结果
- 没有新结果时再回退原大片段
- 拼接前会先统一生成片段的音视频规格，再带着生成音轨一起合成，尽量减少整片音画不同步
- 不再用 mock 结果充数
- 实际拼接时按真实生成文件时长自然串接
