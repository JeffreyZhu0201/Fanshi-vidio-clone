import path from 'node:path';
import { fileURLToPath } from 'node:url';

import dotenv from 'dotenv';
import Joi from 'joi';

const configDirectory = path.dirname(fileURLToPath(import.meta.url));
const backendRoot = path.resolve(configDirectory, '..');

dotenv.config({
  path: path.join(backendRoot, '.env')
});

const schema = Joi.object({
  NODE_ENV: Joi.string().valid('development', 'test', 'production').default('development'),
  PORT: Joi.number().port().default(5000),
  APP_ORIGIN: Joi.string().default('http://localhost:5173'),
  HTTPS_ENABLED: Joi.boolean().truthy('true').falsy('false').default(false),
  HTTPS_PORT: Joi.number().port().default(5443),
  SSL_KEY_PATH: Joi.string().allow('').default(''),
  SSL_CERT_PATH: Joi.string().allow('').default(''),
  HTTP_REDIRECT_TO_HTTPS: Joi.boolean().truthy('true').falsy('false').default(false),
  HTTP_ENABLED: Joi.boolean().truthy('true').falsy('false').default(false),
  DB_HOST: Joi.string().default('127.0.0.1'),
  DB_PORT: Joi.number().port().default(3306),
  DB_USER: Joi.string().default('root'),
  DB_PASSWORD: Joi.string().allow('').default(''),
  DB_NAME: Joi.string().pattern(/^[A-Za-z0-9_]+$/).default('fanshi_video_db'),
  DB_CHARSET: Joi.string().pattern(/^[A-Za-z0-9_]+$/).default('utf8mb4'),
  DB_COLLATION: Joi.string().pattern(/^[A-Za-z0-9_]+$/).default('utf8mb4_unicode_ci'),
  DB_CONNECT_TIMEOUT: Joi.number().integer().positive().default(10000),
  DB_LOGGING: Joi.boolean().truthy('true').falsy('false').default(false),
  DB_POOL_MAX: Joi.number().integer().positive().default(10),
  DB_POOL_MIN: Joi.number().integer().min(0).default(0),
  DB_AUTO_CREATE: Joi.boolean().truthy('true').falsy('false').default(true),
  UPLOAD_BASE_DIR: Joi.string().allow('').default(''),
  PUBLIC_ASSET_BASE_URL: Joi.string().uri().allow('').default(''),
  GEMINI_API_KEY: Joi.string().allow('').default(''),
  GEMINI_API_BASE_URL: Joi.string().uri().allow('').default(''),
  GEMINI_MODEL: Joi.string().default('gemini-2.5-pro'),
  GEMINI_SEGMENT_MODEL: Joi.string().default('gemini-2.5-flash'),
  GEMINI_WHOLE_VIDEO_TIMEOUT_MS: Joi.number().integer().positive().default(600000),
  GEMINI_API_COMPAT_MODE: Joi.string().valid('google', 'openai').default('google'),
  GEMINI_STRICT_REMOTE: Joi.boolean().truthy('true').falsy('false').default(false),
  GEMINI_IMAGE_API_KEY: Joi.string().allow('').default(''),
  GEMINI_IMAGE_API_BASE_URL: Joi.string().uri().allow('').default(''),
  GEMINI_IMAGE_MODEL: Joi.string().default('gemini-3-pro-image-preview'),
  GEMINI_IMAGE_STRICT_REMOTE: Joi.boolean().truthy('true').falsy('false').default(false),
  GEMINI_IMAGE_REQUEST_TIMEOUT: Joi.number().integer().positive().default(120000),
  GEMINI_IMAGE_ASPECT_RATIO: Joi.string().allow('').default(''),
  SEED_DANCE_API_KEY: Joi.string().allow('').default(''),
  SEED_DANCE_API_BASE_URL: Joi.string().uri().allow('').default(''),
  SEED_DANCE_MODEL: Joi.string().default('doubao-seedance-2-0-260128'),
  SEED_DANCE_PUBLIC_ASSET_BASE_URL: Joi.string().uri().allow('').default(''),
  SEED_DANCE_STRICT_REMOTE: Joi.boolean().truthy('true').falsy('false').default(false),
  SEED_DANCE_ALLOW_MOCK_FALLBACK: Joi.boolean().truthy('true').falsy('false').default(false),
  SEED_DANCE_POLL_INTERVAL_MS: Joi.number().integer().positive().default(10000),
  SEED_DANCE_MAX_WAIT_MS: Joi.number().integer().positive().default(900000),
  SEED_DANCE_CREATE_TIMEOUT_MS: Joi.number().integer().positive().default(120000),
  SEED_DANCE_DOWNLOAD_TIMEOUT_MS: Joi.number().integer().positive().default(300000),
  SEED_DANCE_REFERENCE_VIDEO_MAX_DURATION_SECONDS: Joi.number().positive().default(15.2),
  SEED_DANCE_RATIO: Joi.string().default('16:9'),
  SEED_DANCE_DURATION_SECONDS: Joi.number().integer().positive().default(5),
  SEED_DANCE_RESOLUTION: Joi.string().default('720p'),
  SEED_DANCE_GENERATE_AUDIO: Joi.boolean().truthy('true').falsy('false').default(false),
  SEED_DANCE_WATERMARK: Joi.boolean().truthy('true').falsy('false').default(false),
  DOUBAO_SEED_PUBLIC_ASSET_BASE_URL: Joi.string().uri().allow('').default(''),
  SEED_DREAM_API_KEY: Joi.string().allow('').default(''),
  SEED_DREAM_API_BASE_URL: Joi.string().uri().allow('').default(''),
  SEED_DREAM_MODEL: Joi.string().default('doubao-seedream-2-0-260128'),
  EXTERNAL_REQUEST_TIMEOUT: Joi.number().integer().positive().default(30000),
  LOG_LEVEL: Joi.string().valid('error', 'warn', 'info', 'http', 'verbose', 'debug', 'silly').default('info'),
  RATE_LIMIT_WINDOW_MS: Joi.number().integer().positive().default(60000),
  RATE_LIMIT_MAX: Joi.number().integer().positive().default(120),
  JWT_SECRET: Joi.string().min(16).default('development-secret-change-me'),
  FILE_UPLOAD_LIMIT: Joi.number().integer().positive().default(524288000)
}).unknown();

const { error, value } = schema.validate(process.env, {
  abortEarly: false,
  convert: true
});

if (error) {
  throw new Error(
    `Environment validation error:\n${error.details
      .map((detail) => `- ${detail.message}`)
      .join('\n')}`
  );
}

const env = Object.freeze(value);

export default env;
